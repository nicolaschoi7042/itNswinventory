    ...
    >
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Box>
            <Typography variant='h6'>소프트웨어 반납</Typography>
      ...
      <DialogContent dividers>
        ...
        <Alert severity='info' sx={{ mt: 2 }}>
          <Typography variant='body2'>
            반납 완료 후 이 라이선스는 다른 직원에게 재할당할 수 있습니다. 반납
            처리는 취소할 수 없으니 신중히 확인해주세요.
          </Typography>
        </Alert>
      </DialogContent>
      ...
      <DialogActions sx={{ p: 2 }}>
        ...
        <Button
          ...
          disabled={state.submitting}
          startIcon={
            state.submitting ? <CircularProgress size={16} /> : <UndoIcon />
          }
        >
...
export function useSoftwareReturnModal() {
  ...
  const openModal = (
    assignmentData: SoftwareAssignment,
    softwareData: SoftwareWithAssignments
  ) => {
  ...
  return {
    ...
    SoftwareReturnModalComponent: (
      props: Omit<
        SoftwareReturnModalProps,
        'open' | 'onClose' | 'assignment' | 'software'
      >
    ) => (
      <SoftwareReturnModal
        open={open}
