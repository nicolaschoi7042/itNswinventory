...
import { SoftwareReturnModal, useSoftwareReturnModal } from './SoftwareReturnModal';
...
export function SoftwareDetailModal({
  ...
  const loadAssignmentHistory = async () => {
    try {
      ...
      const history = await SoftwareService.getSoftwareAssignmentHistory(software.id);
      setState(prev => ({
        ...prev,
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: error instanceof Error ? error.message : '할당 이력을 불러오는데 실패했습니다.'
      }));
    }
  };
  ...
  const getLicenseStatusDisplay = (status: SoftwareLicenseStatus) => {
    switch (status) {
      case 'active':
        return { color: 'success' as const, icon: <CheckCircleIcon />, text: '활성' };
      case 'expiring_soon':
        return { color: 'warning' as const, icon: <WarningIcon />, text: '만료임박' };
      case 'expired':
        return { color: 'error' as const, icon: <ErrorIcon />, text: '만료' };
      default:
        return { color: 'default' as const, icon: <CheckCircleIcon />, text: '알 수 없음' };
    }
  };
  ...
  const activeAssignments = state.assignmentHistory.filter(a => a.status === '사용중');
  const completedAssignments = state.assignmentHistory.filter(a => a.status === '반납완료');
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Typography variant="body2">
                      {software.purchase_date
                        ? new Date(software.purchase_date).toLocaleDateString('ko-KR')
                        : '-'
                      }
                    </Typography>
                  </Box>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Typography variant="body2">
                        {software.expiry_date
                          ? new Date(software.expiry_date).toLocaleDateString('ko-KR')
                          : '-'
                        }
                      </Typography>
                      {software.expiry_date && (
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Typography variant="body2">
                      {software.created_at
                        ? new Date(software.created_at).toLocaleDateString('ko-KR')
                        : '-'
                      }
                    </Typography>
                  </Box>
                              ...
                              <ManagerGuard>
                                <Tooltip title="반납 처리">
                                  <IconButton
                                    size="small"
                                    color="primary"
                                    onClick={() => handleReturnLicense(assignment)}
                                  >
                      ...
                      <TableBody>
                        {state.assignmentHistory
                          ...
                          <TableRow key={assignment.id}>
                            ...
                            <TableCell>
                              <Typography variant="body2">
                                {assignment.return_date
                                  ? new Date(assignment.return_date).toLocaleDateString('ko-KR')
                                  : '-'
                                }
                              </Typography>
                            </TableCell>
                            ...
                            <TableCell>
                              <Typography variant="body2" color="text.secondary">
                                {assignment.status === '반납완료' && assignment.return_notes
