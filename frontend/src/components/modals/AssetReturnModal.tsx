...
export interface AssetIssue {
  type: 'cosmetic' | 'functional' | 'performance' | 'software' | 'missing_parts';
...
interface AssetReturnModalProps {
  ...
  onClose: () => void;
  onReturn: (returnData: ReturnAssignmentData & { condition: AssetCondition }) => Promise<void>;
  loading?: boolean;
...
const calculateReturnScore = (condition: AssetCondition): number => {
  ...
  const physicalAvg = Object.values(condition.physical).reduce((sum, cond) => 
    sum + (conditionScores[cond as keyof typeof conditionScores] || 0), 0
  ) / Object.values(condition.physical).length;
  
  const functionalAvg = Object.values(condition.functional).reduce((sum, cond) => 
    sum + (conditionScores[cond as keyof typeof conditionScores] || 0), 0
  ) / Object.values(condition.functional).length;

  return (overallScore * weights.overall + physicalAvg * weights.physical + functionalAvg * weights.functional);
};
...
const validateReturn = (
  ...
  const criticalIssues = condition.issues.filter(issue => issue.severity === 'critical');
  const majorIssues = condition.issues.filter(issue => issue.severity === 'major');
  ...
  const estimatedProcessingTime = criticalIssues.length > 0 ? 120 : majorIssues.length > 0 ? 60 : 30;
  const requiresManagerApproval = criticalIssues.length > 0 || returnScore < 2.5;
  ...
  return {
    ...
    approvalReason: requiresManagerApproval ? 
      criticalIssues.length > 0 ? '심각한 자산 손상' : '낮은 자산 상태 점수' : undefined
  };
};
...
export function AssetReturnModal({
  ...
  const [validationResult, setValidationResult] = useState<ReturnValidationResult | null>(null);
  ...
  const handleReturn = async () => {
    ...
    try {
      const completeReturnData: ReturnAssignmentData & { condition: AssetCondition } = {
  ...
  const assignmentDuration = calculateDaysBetween(assignment.assigned_date, returnData.return_date);
  const canProceed = validationResult?.canReturn && 
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    <Typography variant="h6">{assignment.employee_name}</Typography>
              ...
              <Grid item xs={12} md={6}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    <Typography variant="h6">{assignment.asset_description}</Typography>
                    <Typography variant="body2" color="text.secondary">
                      할당일: {formatDate(assignment.assigned_date)} ({assignmentDuration}일 사용)
                    </Typography>
                  </Box>
                </Box>
              </Grid>
        ...
        <Stepper activeStep={activeStep} orientation="vertical">
          {steps.map((step, index) => (
            <Step key={step.id}>
              <StepLabel 
                optional={!step.required && <Typography variant="caption">선택사항</Typography>}
              >
                {step.title}
              </StepLabel>
              <StepContent>
                ...
                {index === 0 && (
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={6}>
                      <DatePicker
                        label="반납일"
                        value={returnData.return_date ? new Date(returnData.return_date) : null}
                    ...
                    <Grid item xs={12} md={6}>
                      <FormControl fullWidth>
                        ...
                        >
                          ...
                          <FormControlLabel value="fair" control={<Radio />} label="보통" />
                          <FormControlLabel value="poor" control={<Radio />} label="불량" />
                          <FormControlLabel value="damaged" control={<Radio />} label="손상" />
                        </RadioGroup>
                      </FormControl>
                    </Grid>
                  </Grid>
                ...
                {index === 1 && requireConditionAssessment && (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    ...
                    <Card variant="outlined">
                      <CardContent>
                        <Typography variant="subtitle1" gutterBottom>전체 평가</Typography>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          <Rating
                            value={assetCondition.rating}
                            onChange={(_, value) => setAssetCondition(prev => ({
                              ...prev,
                              rating: value || 0
                            }))}
                            size="large"
                    ...
                    <Card variant="outlined">
                      <CardContent>
                        <Typography variant="subtitle1" gutterBottom>물리적 상태</Typography>
                        <Grid container spacing={2}>
                          {Object.entries(assetCondition.physical).map(([key, value]) => (
                            <Grid item xs={6} key={key}>
                              <FormControl fullWidth size="small">
                                ...
                                <RadioGroup
                                  row
                                  value={value}
                                  onChange={(e) => setAssetCondition(prev => ({
                                    ...prev,
                                    physical: {
                                      ...prev.physical,
                                      [key]: e.target.value
                                ...
                                >
                                  <FormControlLabel value="excellent" control={<Radio size="small" />} label="우수" />
                                  <FormControlLabel value="good" control={<Radio size="small" />} label="양호" />
                                  <FormControlLabel value="fair" control={<Radio size="small" />} label="보통" />
                                  <FormControlLabel value="poor" control={<Radio size="small" />} label="불량" />
                                </RadioGroup>
                              </FormControl>
                            </Grid>
                          ))}
                    ...
                    <Card variant="outlined">
                      <CardContent>
                        <Typography variant="subtitle1" gutterBottom>기능적 상태</Typography>
                        <Grid container spacing={2}>
                          {Object.entries(assetCondition.functional).map(([key, value]) => (
                            <Grid item xs={6} key={key}>
                              <FormControl fullWidth size="small">
                                <FormLabel>{
                                  key === 'performance' ? '성능' :
                                  key === 'software' ? '소프트웨어' : '연결성'
                                }</FormLabel>
                                <RadioGroup
                                  row
                                  value={value}
                    ...
                    <Card variant="outlined">
                      <CardContent>
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                          <Typography variant="subtitle1">발견된 문제</Typography>
                          <Button
                            variant="outlined"
                            size="small"
                                  ...
                                  <Box sx={{ flex: 1 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                      ...
                                      {issue.requiresRepair && (
                                        <Chip label="수리 필요" size="small" color="warning" />
                                      )}
                                    </Box>
                                    ...
                                    {issue.estimatedCost && (
                                      <Typography variant="caption" color="text.secondary" display="block">
                                        예상 수리비: {issue.estimatedCost.toLocaleString()}원
                                      </Typography>
                                    )}
                                  </Box>
                                  <IconButton
                        ...
                        {showAddIssue && (
                          <Paper sx={{ p: 2, mt: 2, bgcolor: 'background.default' }}>
                            <Typography variant="subtitle2" gutterBottom>새 문제 추가</Typography>
                            <Grid container spacing={2}>
                              <Grid item xs={12} md={6}>
                                <Autocomplete
                                  options={['cosmetic', 'functional', 'performance', 'software', 'missing_parts']}
                                  ...
                                  onChange={(_, value) => setCurrentIssue(prev => ({ ...prev, type: value as any }))}
                                  renderInput={(params) => <TextField {...params} label="문제 유형" size="small" />}
                                />
                              </Grid>
                              <Grid item xs={12} md={6}>
                                <Autocomplete
                                  ...
                                  onChange={(_, value) => setCurrentIssue(prev => ({ ...prev, severity: value as any }))}
                                  renderInput={(params) => <TextField {...params} label="심각도" size="small" />}
                                />
                              </Grid>
                              <Grid item xs={12}>
                                <TextField
                              ...
                              <Grid item xs={12} md={6}>
                                <TextField
                                  ...
                                  onChange={(e) => setCurrentIssue(prev => ({ ...prev, location: e.target.value }))}
                                />
                              </Grid>
                              <Grid item xs={12} md={6}>
                                <TextField
                                  ...
                                  onChange={(e) => setCurrentIssue(prev => ({ 
                                    ...prev, 
                              ...
                              <Grid item xs={12}>
                                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                  <FormControlLabel
                                    control={
                                      <Switch
                                        checked={currentIssue.requiresRepair || false}
                                        onChange={(e) => setCurrentIssue(prev => ({ 
                                          ...prev, 
                ...
                {index === 3 && validationResult && (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {/* Validation Results */}
                    <Alert 
                      severity={validationResult.canReturn ? 'success' : 'error'}
                      icon={validationResult.canReturn ? <CheckCircleIcon /> : <ErrorIcon />}
                    >
                      <Typography variant="subtitle2">
                        {validationResult.canReturn ? '반납 처리 가능' : '반납 처리 불가'}
                      </Typography>
                      <Typography variant="body2">
                        예상 처리 시간: {validationResult.estimatedProcessingTime}분
                      </Typography>
                    </Alert>
