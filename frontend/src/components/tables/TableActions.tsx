...
export function TableActions({
  ...
  const filteredActions = actions.filter(action =>
    hasRole(action.requiresRole || 'user')
  );
  const filteredRowActions = rowActions.filter(action =>
    hasRole(action.requiresRole || 'user')
  );
  ...
  if (actions.length > 0 && !rowData) {
    ...
    const menuActions =
      variant === 'mixed' ? filteredActions.slice(maxVisibleActions) : [];
    ...
    return (
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
        ...
        {(variant === 'menu' ||
          (variant === 'mixed' && menuActions.length > 0)) && (
          <>
            <IconButton
              size={dense ? 'small' : 'medium'}
            ...
            >
              {(variant === 'menu' ? filteredActions : menuActions).map(
                (action, index) => (
                  <MenuItem
                    key={index}
                    onClick={() => {
...
export const commonTableActions = {
  add: (
    onClick: () => void,
    requiresRole: UserRole | UserRole[] = 'manager'
  ): TableAction => ({
    label: 'Add New',
    icon: <AddIcon />,
    onClick,
...
export const commonRowActions = {
  ...
  delete: (
    onClick: (rowData?: any, index?: number) => void,
    requiresRole: UserRole | UserRole[] = 'admin'
  ): RowAction => ({
    label: 'Delete',
    icon: <DeleteIcon />,
    onClick,
