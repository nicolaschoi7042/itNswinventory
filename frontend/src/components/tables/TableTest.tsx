...
const generateHardwareData = (count: number) => {
  ...
  return Array.from({ length: count }, (_, index) => ({
    ...
    assignedTo:
      Math.random() > 0.3 ? `직원${Math.floor(Math.random() * 100) + 1}` : null,
...
export function TableTest() {
  ...
  const [currentTab, setCurrentTab] = useState<'employee' | 'hardware'>(
    'employee'
  );
  ...
  const [employeeData, setEmployeeData] = useState(() =>
    generateEmployeeData(50)
  );
  const [hardwareData, setHardwareData] = useState(() =>
    generateHardwareData(30)
  );
  ...
  const statusOptions = [...new Set(employeeData.map(emp => emp.status))].map(
    status => ({
      value: status,
      label: status,
    })
  ...
  const typeOptions = [...new Set(hardwareData.map(hw => hw.type))].map(
    type => ({
      value: type,
      label: type,
    })
  ...
  useEffect(() => {
    setTestMetrics(prev => ({
      ...prev,
      rowCount:
        currentTab === 'employee' ? employeeData.length : hardwareData.length,
    }));
            ...
            <CardContent>
              <Stack spacing={2}>
                ...
                <Stack direction='row' spacing={2} alignItems='center'>
                  ...
                  <Button
                    variant={
                      currentTab === 'employee' ? 'contained' : 'outlined'
                    }
                    size='small'
                  ...
                  <Button
                    variant={
                      currentTab === 'hardware' ? 'contained' : 'outlined'
                    }
                    size='small'
            ...
            <CardContent>
              <Stack spacing={2}>
                ...
                <Box>
                  ...
                  <Typography
                    variant='h6'
                    color={testMetrics.renderTime > 100 ? 'error' : 'success'}
                  >
                ...
                <Box>
                  ...
                  <Typography
                    variant='h6'
                    color={testMetrics.filterTime > 50 ? 'warning' : 'success'}
                  >
                ...
                <Alert
                  severity={
                    testMetrics.renderTime < 100
                      ? 'success'
                      : testMetrics.renderTime < 300
                        ? 'warning'
                        : 'error'
                  }
                  icon={
                    testMetrics.renderTime < 100 ? <CheckIcon /> : <ErrorIcon />
                  }
                >
