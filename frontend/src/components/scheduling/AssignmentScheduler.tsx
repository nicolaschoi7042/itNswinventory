...
const generateTimeSlots = (
  ...
  for (let hour = 9; hour <= 18; hour++) {
    ...
    const conflicts = assignments.filter(assignment => {
      ...
      return (
        assignedDate.toDateString() === targetDate.toDateString() &&
...
const checkSchedulingConflicts = (
  ...
  const employeeAssignments = assignments.filter(
    assignment =>
      ...
      assignment.status === '사용중' &&
      new Date(assignment.assigned_date).toDateString() ===
...
export function AssignmentScheduler({
  ...
  const [scheduledAssignment, setScheduledAssignment] = useState<
    Partial<ScheduledAssignment>
  >({
  ...
  useEffect(() => {
    if (
      ...
      const newConflicts = checkSchedulingConflicts(
        scheduledAssignment,
        assignments
      );
  ...
  const handleSchedule = () => {
    ...
    const fullSchedule: ScheduledAssignment = {
      ...
      employeeName:
        employees.find(e => e.id === scheduledAssignment.employeeId)?.name ||
        '',
      assetId: scheduledAssignment.assetId,
      assetName:
        assets.find(a => a.id === scheduledAssignment.assetId)?.name || '',
      assetType:
        assets.find(a => a.id === scheduledAssignment.assetId)?.type ===
        'software'
          ? 'software'
          : 'hardware',
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <Autocomplete
                ...
                value={
                  employees.find(
                    e => e.id === scheduledAssignment.employeeId
                  ) || null
                }
                onChange={(_, value) =>
                  handleFieldChange('employeeId', value?.id || '')
                }
                renderInput={params => (
            ...
            <Grid item xs={12} md={6}>
              <Autocomplete
                ...
                value={
                  assets.find(a => a.id === scheduledAssignment.assetId) || null
                }
                onChange={(_, value) =>
                  handleFieldChange('assetId', value?.id || '')
                }
                renderInput={params => (
            ...
            <Grid item xs={12} md={4}>
              <DatePicker
                ...
                value={
                  scheduledAssignment.scheduledDate
                    ? new Date(scheduledAssignment.scheduledDate)
                    : null
                }
                onChange={date =>
                  handleFieldChange(
                ...
                maxDate={
                  new Date(
                    Date.now() + maxFutureMonths * 30 * 24 * 60 * 60 * 1000
                  )
                }
              />
            </Grid>
            ...
            <Grid item xs={12} md={4}>
              <DatePicker
                label='예상 반납일 (선택사항)'
                value={
                  scheduledAssignment.returnDate
                    ? new Date(scheduledAssignment.returnDate)
                    : null
                }
                onChange={date =>
                  handleFieldChange(
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={e =>
                          handleFieldChange('autoAssign', e.target.checked)
                        }
                      />
                    }
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            ...
                            onChange={e =>
                              handleFieldChange('recurring', e.target.checked)
                            }
                          />
                        }
                        label='반복 일정'
                        ...
                        <Grid container spacing={2} sx={{ mt: 1 }}>
                          ...
                          <Grid item xs={6}>
                            <DatePicker
                              label='반복 종료일'
                              value={
                                scheduledAssignment.recurringEnd
                                  ? new Date(scheduledAssignment.recurringEnd)
                                  : null
                              }
                              onChange={date =>
                                handleFieldChange(
                    ...
                    <Stack spacing={1}>
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            checked={
                              scheduledAssignment.notifications
                      ...
                      {scheduledAssignment.notifications?.emailReminder && (
                        <TextField
                          ...
                          label='알림 시간 (분 전)'
                          value={
                            scheduledAssignment.notifications.reminderBefore
                          }
                          onChange={e =>
                            handleNotificationChange(
                              'reminderBefore',
