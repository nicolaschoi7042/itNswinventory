...
export function StatusFilter({
  ...
  const [filterMode, setFilterMode] = useState<'individual' | 'groups'>('individual');

  // Calculate status distribution
  const statusDistribution = useMemo(() => 
    calculateStatusDistribution(assignments), [assignments]
  );

  // Handle status selection
  const handleStatusToggle = useCallback((status: AssignmentStatus) => {
    if (allowMultiple) {
  ...
  const handleGroupSelect = useCallback((group: StatusGroup) => {
    ...
    if (allSelected) {
      ...
      const newStatuses = selectedStatuses.filter(status => !groupStatuses.includes(status));
      onStatusChange(newStatuses);
    } else {
      // Add all group statuses
      const newStatuses = [...new Set([...selectedStatuses, ...groupStatuses])];
      onStatusChange(newStatuses);
    }
  ...
  if (variant === 'compact') {
    return (
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {statusDistribution.filter(dist => dist.count > 0).map((dist) => (
          <Chip
            key={dist.status}
            icon={dist.icon}
            label={`${ASSIGNMENT_STATUS_LABELS[dist.status]} (${dist.count})`}
            variant={selectedStatuses.includes(dist.status) ? 'filled' : 'outlined'}
            color={selectedStatuses.includes(dist.status) ? (dist.color as any) : 'default'}
            size="small"
            onClick={() => handleStatusToggle(dist.status)}
            sx={{
              '&:hover': {
                backgroundColor: alpha(
                  theme.palette[dist.color as keyof typeof theme.palette]?.main || theme.palette.primary.main,
                  0.1
                )
              }
            }}
      ...
      <CardContent>
        ...
        <Box sx={{ mb: 3, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          <Button
            ...
            disabled={selectedStatuses.length === Object.values(ASSIGNMENT_STATUSES).length}
          >
            전체 선택
          </Button>
          <Button
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      backgroundColor: selectedStatuses.includes(dist.status) 
                        ? alpha(theme.palette[dist.color as keyof typeof theme.palette]?.main || theme.palette.primary.main, 0.1)
                        : 'transparent',
                      '&:hover': {
                        backgroundColor: alpha(
                          theme.palette[dist.color as keyof typeof theme.palette]?.main || theme.palette.primary.main,
            ...
            <Box>
              ...
              <Stack spacing={2}>
                {STATUS_GROUPS.map((group) => {
                  const groupStatuses = group.statuses;
                  const allSelected = groupStatuses.every(status => selectedStatuses.includes(status));
                  const someSelected = groupStatuses.some(status => selectedStatuses.includes(status));
                    ...
                    <Card
                      ...
                      sx={{
                        ...
                        backgroundColor: allSelected 
                          ? alpha(theme.palette[group.color as keyof typeof theme.palette]?.main || theme.palette.primary.main, 0.1)
                          : 'transparent',
                        '&:hover': {
                          backgroundColor: alpha(
                            theme.palette[group.color as keyof typeof theme.palette]?.main || theme.palette.primary.main,
                            0.05
                          )
                        }
                      }}
                    >
                      <CardContent sx={{ py: 1.5 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <Box sx={{ textAlign: 'right' }}>
                            <Typography variant="h6" color={`${group.color}.main`}>
                              {groupCount}
                            </Typography>
            ...
            >
              <Box
                ...
                sx={{
                  ...
                  backgroundColor: selectedStatuses.includes(dist.status)
                    ? theme.palette[dist.color as keyof typeof theme.palette]?.main
                    : alpha(theme.palette[dist.color as keyof typeof theme.palette]?.main || theme.palette.primary.main, 0.3),
                  borderRadius: '4px 4px 0 0',
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  '&:hover': {
                    backgroundColor: theme.palette[dist.color as keyof typeof theme.palette]?.main,
