...
const createQuickFilters = (assignments: (Assignment | AssignmentWithDetails)[]): QuickFilter[] => [
  {
    id: 'active',
    label: '사용 중',
    icon: <PersonIcon />,
  ...
  },
  {
    ...
    icon: <ScheduleIcon />,
    filters: { 
      assigned_date_from: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    },
    color: 'primary'
  },
  {
...
export function AssignmentFilters({
  ...
  // Quick filters
  const quickFilters = useMemo(() => createQuickFilters(assignments), [assignments]);
  ...
  const handleQuickFilter = useCallback((quickFilter: QuickFilter) => {
    ...
    const isActive = Object.entries(quickFilter.filters).every(([key, value]) => 
      filters[key as keyof IAssignmentFilters] === value
    );
  ...
  if (variant === 'compact') {
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
        ...
        {/* Quick filters */}
        {showQuickFilters && quickFilters.slice(0, 3).map((filter) => (
          <Chip
            key={filter.id}
            icon={filter.icon}
            label={filter.label}
            variant={isQuickFilterActive(filter) ? "filled" : "outlined"}
            color={isQuickFilterActive(filter) ? (filter.color as any) : "default"}
                      ...
                      sx={{
                        '&:hover': {
                          backgroundColor: alpha(
                            theme.palette[filter.color as keyof typeof theme.palette]?.main || theme.palette.primary.main,
                            0.1
                          )
                        }
                      }}
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Autocomplete
                ...
                value={uniqueEmployees.find(emp => emp === filters.employee_id) || null}
                onChange={(_, value) => handleFilterChange('employee_id', value)}
                renderInput={(params) => (
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Autocomplete
                ...
                value={uniqueAssetIds.find(id => id === filters.asset_id) || null}
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Autocomplete
                ...
                value={uniqueManufacturers.find(mfg => mfg === filters.manufacturer) || null}
                onChange={(_, value) => handleFilterChange('manufacturer', value)}
                renderInput={(params) => (
