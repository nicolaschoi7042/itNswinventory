...
const generateRealTimeActivity = (): Activity => {
  ...
  const actions = ['자산 등록', '자산 수정', '자산 삭제', '할당 변경', '직원 추가', '라이센스 갱신'];
  const types: Activity['targetType'][] = ['hardware', 'software', 'employee', 'assignment'];
...
const generateDynamicStats = () => {
  ...
  return {
    ...
    software: Math.max(0, Math.min(100, baseStats.software + (Math.random() * 10 - 5))),
...
export function DashboardTest() {
  ...
  const [testMode, setTestMode] = useState<'static' | 'dynamic' | 'stress'>('static');
  ...
  const [currentStats, setCurrentStats] = useState(() => generateDynamicStats());
  const [activities, setActivities] = useState<Activity[]>([]);
  const [assetData, setAssetData] = useState<AssetData[]>(() => generateAssetChartData());
  ...
  useEffect(() => {
    ...
    const updateInterval = testMode === 'stress' ? 500 : 
                          testMode === 'dynamic' ? 2000 : 5000;
  ...
  const handleTestModeChange = (mode: typeof testMode) => {
    ...
    if (mode === 'stress') {
      ...
      const stressActivities = Array.from({ length: 50 }, () => generateRealTimeActivity());
        ...
        <CardContent>
          <Grid container spacing={3} alignItems="center">
            <Grid item xs={12} md={6}>
              <Typography variant="body2" sx={{ mb: 1 }}>테스트 모드:</Typography>
              <Stack direction="row" spacing={1}>
                {(['static', 'dynamic', 'stress'] as const).map((mode) => (
                  ...
                  >
                    {mode === 'static' ? '정적' : mode === 'dynamic' ? '동적' : '스트레스'}
                  </Button>
                ))}
              </Stack>
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <Stack direction="row" spacing={2} alignItems="center">
                <Box>
                  <Typography variant="caption" color="text.secondary">업데이트 횟수</Typography>
                  <Typography variant="h6">{metrics.updateCount}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary">평균 렌더링 시간</Typography>
                  <Typography variant="h6" color={metrics.renderTime > 100 ? 'error' : 'success'}>
                    {metrics.renderTime}ms
                  </Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary">오류 수</Typography>
                  <Typography variant="h6" color={metrics.errorCount > 0 ? 'error' : 'text.primary'}>
                    {metrics.errorCount}
                  </Typography>
                </Box>
                <Button size="small" onClick={resetMetrics}>초기화</Button>
              </Stack>
            </Grid>
          </Grid>
          ...
          <Card>
            ...
            <CardContent>
              <Grid container spacing={3}>
                <Grid item xs={12} md={3}>
                  <Box textAlign="center">
                    <Typography variant="h4" color={metrics.renderTime > 100 ? 'error' : 'success'}>
                      {metrics.renderTime}ms
                    </Typography>
                ...
                <Grid item xs={12} md={3}>
                  <Box textAlign="center">
                    <Typography variant="h4" color={metrics.errorCount > 0 ? 'error' : 'success'}>
                      {metrics.errorCount}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      오류 발생 횟수
              ...
              <Box sx={{ mt: 3 }}>
                <Alert 
                  severity={
                    metrics.errorCount > 0 ? 'error' :
                    metrics.renderTime > 100 ? 'warning' :
                    metrics.updateCount > 0 ? 'success' : 'info'
                  }
                  icon={
                    metrics.errorCount > 0 ? <ErrorIcon /> :
                    metrics.renderTime > 100 ? <WarningIcon /> :
                    <CheckIcon />
                  }
                >
              ...
              <Box sx={{ mt: 2 }}>
                ...
                <Typography variant="caption" color="text.secondary" component="div">
                  • <strong>정적 모드:</strong> 고정된 데이터로 기본 렌더링 테스트<br/>
                  • <strong>동적 모드:</strong> 2초마다 데이터 업데이트로 실시간 성능 테스트<br/>
                  • <strong>스트레스 모드:</strong> 0.5초마다 대량 데이터 업데이트로 극한 성능 테스트
                </Typography>
              </Box>
            </CardContent>
          </Card>
