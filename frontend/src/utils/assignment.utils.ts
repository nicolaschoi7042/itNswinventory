 ...
 */
export const shouldMarkAsOverdue = (
...
export const validateAssetAvailability = (
  ...
  const activeAssignments = assignments.filter(
    assignment =>
      assignment.asset_id === assetId && assignment.status === '사용중'
  );
...
export const validateEmployeeAssignmentLimits = (
  ...
  const activeAssignments = assignments.filter(
    assignment =>
      assignment.employee_id === employeeId && assignment.status === '사용중'
  );
...
export const validateSoftwareLicenseAvailability = (
  ...
  const maxLicenses =
    softwareData?.max_licenses || softwareData?.total_licenses || 1;
...
export const validateAssignmentEligibility = (
  ...
  const assetAvailability = validateAssetAvailability(
    assetId,
    assetType,
    filteredAssignments
  );
  ...
  if (!employeeLimits.canAssign) {
    ...
    employeeLimits.currentCount >=
    (options?.maxEmployeeAssignments || 5) - 1
  ) {
    warnings.push(
      `직원의 할당 한도에 거의 도달했습니다. (${employeeLimits.currentCount + 1}/${options?.maxEmployeeAssignments || 5})`
    );
  }
  ...
  if (assetType === 'software' && options?.softwareData) {
    ...
    if (!licenseValidation.isAvailable) {
      ...
      licenseValidation.utilizationRate &&
      licenseValidation.utilizationRate >= 80
    ) {
      warnings.push(
        `소프트웨어 라이선스 사용률이 높습니다. (${licenseValidation.utilizationRate.toFixed(1)}%)`
      );
    }
  }

  // 4. Check for scheduling conflicts
  const schedulingConflicts = assignments.filter(
    assignment =>
  ...
  if (issues.length === 0) {
    ...
    if (issues.some(i => i.type === 'asset_availability')) {
      recommendations.push(
        '다른 사용 가능한 자산을 선택하거나 기존 할당이 반납된 후 다시 시도하세요.'
      );
    }
    if (issues.some(i => i.type === 'employee_limit')) {
      recommendations.push(
        '직원의 기존 할당을 반납하거나 관리자에게 한도 증가를 요청하세요.'
      );
    }
  }
...
export const validateCreateAssignmentData = (
  ...
  if (
    data.notes &&
    data.notes.length > ASSIGNMENT_VALIDATION_RULES.NOTES_MAX_LENGTH
  ) {
    errors.push(
      `메모는 ${ASSIGNMENT_VALIDATION_RULES.NOTES_MAX_LENGTH}자 이하로 입력해주세요.`
    );
  }

  // Enhanced availability and eligibility validation
  let eligibilityCheck:
    | ReturnType<typeof validateAssignmentEligibility>
    | undefined;
...
export const validateReturnAssignmentData = (
  ...
  if (
    data.return_notes &&
    data.return_notes.length >
      ASSIGNMENT_VALIDATION_RULES.RETURN_NOTES_MAX_LENGTH
  ) {
    errors.push(
      `반납 메모는 ${ASSIGNMENT_VALIDATION_RULES.RETURN_NOTES_MAX_LENGTH}자 이하로 입력해주세요.`
    );
  }
...
export const applyAdvancedStatusFilters = (
  ...
  if (
    statusConditions.includeStatuses &&
    statusConditions.includeStatuses.length > 0
  ) {
    filtered = filtered.filter(a =>
      statusConditions.includeStatuses!.includes(a.status)
    );
  }

  // Exclude specific statuses
  if (
    statusConditions.excludeStatuses &&
    statusConditions.excludeStatuses.length > 0
  ) {
    filtered = filtered.filter(
      a => !statusConditions.excludeStatuses!.includes(a.status)
    );
  }
  ...
  if (statusConditions.dateRange) {
    ...
    if (from) {
      ...
      filtered = filtered.filter(a => {
        const dateValue =
          field === 'assigned_date' ? a.assigned_date : a.return_date;
...
export const calculateStatusChangeImpact = (
  ...
  if (affectedCount > 10) {
    warnings.push(
      `${affectedCount}개의 할당이 영향을 받습니다. 일괄 처리를 신중하게 진행하세요.`
    );
  }
 ...
 */
export const getAssignmentTrends = (
  assignments: Assignment[],
  days: number = 30
): {
  dates: string[];
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const assignedCount = assignments.filter(
      a => formatDateForAPI(new Date(a.assigned_date)) === dateStr
    ).length;
 ...
 */
export const prepareAssignmentDataForExport = (
  assignments: Assignment[]
): Record<string, any>[] => {
...
export const generateExportFilename = (
  type: 'assignments' | 'history' | 'stats',
