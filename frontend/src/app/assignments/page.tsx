...
import { AssignmentFilters as AssignmentFiltersComponent, QuickFilterBar } from '@/components/filters/AssignmentFilters';
...
export default function AssignmentsPage() {
  ...
  const [state, setState] = useState<AssignmentPageState>({
    assignments: [],
    employees: [
      { id: 'E001', name: '김철수', department: 'IT개발팀', position: '시니어 개발자', email: 'kim@company.com' },
      { id: 'E002', name: '이영희', department: '마케팅팀', position: '매니저', email: 'lee@company.com' },
      { id: 'E003', name: '박민수', department: '디자인팀', position: '선임 디자이너', email: 'park@company.com' },
      { id: 'E004', name: '정수진', department: '마케팅팀', position: '주임', email: 'jung@company.com' },
      { id: 'E005', name: '최영호', department: '영업팀', position: '과장', email: 'choi@company.com' },
      { id: 'E006', name: '한미래', department: '영업팀', position: '사원', email: 'han@company.com' }
    ],
    hardware: [
      { id: 'HW001', name: 'Dell OptiPlex 7090', type: 'Desktop', manufacturer: 'Dell', model: 'OptiPlex 7090', status: 'available' },
      { id: 'HW002', name: 'MacBook Pro 16"', type: 'Laptop', manufacturer: 'Apple', model: 'MacBook Pro 16" M2', status: 'available' },
      { id: 'HW003', name: 'Samsung Monitor 27"', type: 'Monitor', manufacturer: 'Samsung', model: 'S27E450D', status: 'available' },
      { id: 'HW004', name: 'iPhone 15 Pro', type: 'Mobile', manufacturer: 'Apple', model: 'iPhone 15 Pro 256GB', status: 'available' },
      { id: 'HW005', name: 'Logitech Keyboard', type: 'Peripheral', manufacturer: 'Logitech', model: 'MX Keys', status: 'available' }
    ],
    software: [
      { id: 'SW001', name: 'Microsoft Office 365', type: 'Office Suite', manufacturer: 'Microsoft', version: 'Office 365 Business Premium', status: 'available' },
      { id: 'SW002', name: 'Adobe Creative Suite', type: 'Design Software', manufacturer: 'Adobe', version: 'Creative Cloud 2024', status: 'available' },
      { id: 'SW003', name: 'Visual Studio Code', type: 'Development Tool', manufacturer: 'Microsoft', version: 'Latest', status: 'available' },
      { id: 'SW004', name: 'Slack', type: 'Communication', manufacturer: 'Slack Technologies', version: 'Business+', status: 'available' }
    ],
    loading: true,
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return result;
  }, [state.assignments, state.searchQuery, state.filters, state.sortBy, state.sortOrder]);
  ...
  const handleDeleteAssignment = async (assignment: AssignmentWithDetails) => {
    ...
    const confirmMessage = `할당 "${assignment.id}"을 정말 삭제하시겠습니까?\n\n` +
    ...
    try {
      ...
      const updatedAssignments = state.assignments.filter(assign => assign.id !== assignment.id);
          ...
          setState(prev => ({
            ...
            hardware: prev.hardware.map(hw => 
              hw.id === assignment.asset_id 
                ? { ...hw, status: 'available', updated_at: new Date().toISOString() }
                : hw
            )
          }));
        } else {
          setState(prev => ({
            ...
            software: prev.software.map(sw => 
              sw.id === assignment.asset_id 
                ? { ...sw, status: 'available', updated_at: new Date().toISOString() }
                : sw
            )
          }));
        }
        ...
        setState(prev => ({
          ...
          employees: prev.employees.map(emp => 
            emp.id === assignment.employee_id
              ? { 
                  ...emp, 
                  active_assignments: Math.max((emp.active_assignments || 1) - 1, 0),
  ...
  const handleAssetReturn = async (returnData: any) => {
    try {
      ...
      console.log('Calling AssignmentService.returnAsset:', assignment.id, apiReturnData);
      const response = await assignmentService.returnAsset(assignment.id, apiReturnData);
      ...
      const employee = state.employees.find(e => e.id === assignment.employee_id);
      const asset = assignment.asset_type === 'hardware' 
      ...
      setState(prev => ({
        ...
        successMessage: `자산 "${assignment.asset_description}"이(가) 성공적으로 반납되었습니다.${
          returnData.condition === 'damaged' ? ' 자산이 정비 상태로 변경되었습니다.' : ''
        }`
      }));
  ...
  const handleSortChange = (sortBy: keyof Assignment, sortOrder: 'asc' | 'desc') => {
  ...
  const handleExportExcel = async (exportType: 'all' | 'filtered' = 'filtered') => {
    try {
      ...
      setState(prev => ({
        ...
        successMessage: `Excel 파일이 성공적으로 다운로드되었습니다. (총 ${
          exportType === 'all' ? state.assignments.length : filteredAssignments.length
        }개 항목)`
      }));
      ...
      logError('Excel Export', error, { 
        exportType,
        itemCount: exportType === 'all' ? state.assignments.length : filteredAssignments.length
      });
  ...
  const handleSubmitAssignment = async (data: CreateAssignmentData | UpdateAssignmentData) => {
    try {
      setState(prev => ({ ...prev, loading: true }));

      if (state.selectedAssignment) {
        // Update existing assignment via API
        console.log('Updating assignment via API:', state.selectedAssignment.id, data);
        
        const response = await AssignmentService.update(state.selectedAssignment.id, data);
        
        if (response.success && response.data) {
          // Enrich the updated assignment with employee and asset details
          const employee = state.employees.find(e => e.id === response.data.employee_id);
          const asset = response.data.asset_type === 'hardware' 
            ? state.hardware.find(h => h.id === response.data.asset_id)
            : state.software.find(s => s.id === response.data.asset_id);

          const enrichedAssignment: AssignmentWithDetails = {
            ...response.data,
            employee_name: employee?.name || state.selectedAssignment?.employee_name || '',
            asset_description: asset?.name || state.selectedAssignment?.asset_description || '',
          ...
          const updatedAssignments = state.assignments.map(assignment => 
            assignment.id === state.selectedAssignment?.id ? enrichedAssignment : assignment
          );
          ...
          if (oldAssignment) {
            ...
            if (oldAssignment.asset_id !== response.data.asset_id || oldAssignment.asset_type !== response.data.asset_type) {
              // Free up old asset
                ...
                setState(prev => ({
                  ...
                  hardware: prev.hardware.map(hw => 
                    hw.id === response.data.asset_id 
                      ? { ...hw, status: 'assigned', updated_at: new Date().toISOString() }
                      : hw
                  )
                }));
              } else {
                setState(prev => ({
                  ...
                  software: prev.software.map(sw => 
                    sw.id === response.data.asset_id 
                      ? { ...sw, status: 'assigned', updated_at: new Date().toISOString() }
                      : sw
                  )
                }));
              }
        ...
        if (response.success && response.data) {
          ...
          const asset = data.asset_type === 'hardware' 
  ...
  const filterOptions = {
    ...
    department: Array.from(new Set(state.assignments.map(a => a.employee.department))).map(dept => ({
      value: dept,
      label: dept
    })),
      ...
      <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ mt: 2 }}>
              <Alert severity="info" variant="outlined">
                <Typography variant="body2">
                  현재 읽기 전용 모드입니다. 할당 생성, 수정, 삭제 권한이 필요한 경우 관리자에게 문의하세요.
                </Typography>
              </Alert>
            </Box>
          )}
        ...
        <Box sx={{ mb: 3 }}>
          <QuickFilterBar
            ...
            onFiltersChange={(filters) => setState(prev => ({ 
              ...prev, 
              filters, 
              page: 1 
            }))}
            onAdvancedSearch={() => setState(prev => ({ ...prev, showFilterPanel: true }))}
          />
        </Box>
        ...
        <Box sx={{ 
          ...
          <Button
            ...
            onClick={(event) => setState(prev => ({ ...prev, anchorEl: event.currentTarget }))}
          >
            내보내기
          </Button>
        ...
        <AssignmentDetailModal
          ...
          assignment={state.selectedAssignment}
          onClose={() => setState(prev => ({ ...prev, showDetailModal: false, selectedAssignment: null }))}
        ...
        <AdvancedSearchModal
          ...
          onClose={() => setState(prev => ({ ...prev, showAdvancedSearch: false }))}
