...
import { SoftwareAssignmentModal, useSoftwareAssignmentModal } from '@/components/modals/SoftwareAssignmentModal';
...
export default function SoftwarePage() {
  ...
  const loadSoftware = useCallback(async () => {
    try {
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: error instanceof Error ? error.message : '소프트웨어 데이터를 불러오는데 실패했습니다.'
      }));
    }
  }, []);
  ...
  const filteredSoftware = useMemo(() => {
    ...
    if (state.filters.manufacturer) {
      result = result.filter(software => software.manufacturer === state.filters.manufacturer);
    }

    if (state.filters.license_type) {
      result = result.filter(software => software.license_type === state.filters.license_type);
    }

    if (state.filters.status) {
      result = result.filter(software => getSoftwareLicenseStatus(software) === state.filters.status);
    }
    ...
    if (state.filters.utilization_filter) {
      result = result.filter(software => {
        ...
        switch (state.filters.utilization_filter) {
          case 'low': return utilization.percentage < 50;
          case 'medium': return utilization.percentage >= 50 && utilization.percentage < 80;
          case 'high': return utilization.percentage >= 80 && utilization.percentage < 100;
          case 'full': return utilization.percentage === 100;
          default: return true;
        }
      });
    }
    ...
    if (state.filters.purchase_date_from) {
      result = result.filter(software => {
        ...
        return new Date(software.purchase_date) >= new Date(state.filters.purchase_date_from!);
      });
    }
    ...
    if (state.filters.purchase_date_to) {
      result = result.filter(software => {
        if (!software.purchase_date) return false;
        return new Date(software.purchase_date) <= new Date(state.filters.purchase_date_to!);
      });
    }
    ...
    if (state.filters.expiry_date_from) {
      result = result.filter(software => {
        ...
        return new Date(software.expiry_date) >= new Date(state.filters.expiry_date_from!);
      });
    }
    ...
    if (state.filters.expiry_date_to) {
      result = result.filter(software => {
        if (!software.expiry_date) return false;
        return new Date(software.expiry_date) <= new Date(state.filters.expiry_date_to!);
      });
    }
          ...
          <ManagerGuard>
            <Tooltip title="할당">
              <IconButton
                ...
                disabled={calculateLicenseUtilization(software).available <= 0}
              >
  ...
  const handleDeleteSoftware = async (software: SoftwareWithAssignments) => {
    if (!window.confirm(`"${software.name}" 소프트웨어를 정말 삭제하시겠습니까?`)) {
      return;
    }
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '소프트웨어 삭제에 실패했습니다.'
      }));
    }
  };
  ...
  const handleSoftwareSaved = () => {
    setState(prev => ({
      ...
      selectedSoftware: null,
      editMode: false,
      successMessage: state.editMode ? '소프트웨어가 수정되었습니다.' : '소프트웨어가 등록되었습니다.'
    }));
    loadSoftware();
  };
  ...
  const handleExportExcel = async () => {
    try {
      await SoftwareService.downloadExcel(
        state.searchQuery ? { query: state.searchQuery, filters: state.filters } : { filters: state.filters }
      );
      setState(prev => ({
        ...prev,
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : 'Excel 내보내기에 실패했습니다.'
      }));
    }
  };
  ...
  const handleExportUtilizationReport = async () => {
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '라이선스 사용률 보고서 다운로드에 실패했습니다.'
      }));
    }
  };
  ...
  const handleExportAssignmentHistory = async () => {
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error.message : '할당 이력 다운로드에 실패했습니다.'
      }));
    }
  };
  ...
  // Filter options
  const filterOptions = {
    type: Array.from(new Set(state.software.map(s => s.type).filter(Boolean))),
    manufacturer: Array.from(new Set(state.software.map(s => s.manufacturer).filter(Boolean))),
    license_type: Array.from(new Set(state.software.map(s => s.license_type).filter(Boolean))),
    status: [
    ...
    utilization_filter: [
      { value: 'low', label: '낮음 (<50%)' },
      { value: 'medium', label: '보통 (50-80%)' },
      { value: 'high', label: '높음 (80-99%)' },
      { value: 'full', label: '완전 (100%)' }
    ],
    // Date range filters
    purchase_date_from: true, // Enable date picker
    purchase_date_to: true,
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ 
        ...
        <SearchFilter
          ...
          placeholder="소프트웨어명, 제조사, 버전으로 검색..."
        />
      </Box>
        ...
        <Card sx={{ minWidth: 200 }}>
          <CardContent>
            ...
            <Typography variant="h5" color="warning.main">
              {state.software.filter(s => getSoftwareLicenseStatus(s) === 'expiring_soon').length}
            </Typography>
          </CardContent>
        </Card>
      </Box>
